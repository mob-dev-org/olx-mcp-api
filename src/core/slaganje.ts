...
/**
 * Da li je slaganje uopste moguce na ovoj masini. Zavisnosti su teske (onnxruntime je preko
 * 100 MB) i instaliraju se sa npm install; na klonu gdje ih nema, poruka kaze tacno sta fali.
 */
export async function slaganjeDostupno(): Promise<{ ok: true } | { ok: false; razlog: string }> {
  try {
    await import("sharp");
  } catch {
    return { ok: false, razlog: "sharp nije instaliran; pokreni npm install u klonu" };
  }
  try {
    await import("@imgly/background-removal-node");
  } catch {
    return { ok: false, razlog: "@imgly/background-removal-node nije instaliran; pokreni npm install u klonu" };
  }
  return { ok: true };
}
