...
| `olx_opisi_sliku` | Anthropic Messages API (`core/vid.ts`, model iz `OLX_VID_MODEL`, default claude-haiku-4-5), NE zove OLX | Vision proxy: opis slike sa diska za sesije ciji pogon nema vid (DeepSeek slike ignorise). Registruje se SAMO kad je `OLX_VID_API_KEY` postavljen. Trosak ide u `.olx-pik/ai-usage.jsonl`, ne u kredite. | `putanja` | `pitanje` (default: opis proizvoda za oglas) | read (poziv vanjskog AI-ja) | ne | ne |
