...
| `olx_generiraj_sliku` | Google Gemini `generateContent` (`core/slika.ts` + `core/gemini.ts`, model iz `OLX_SLIKA_MODEL`, default gemini-3.1-flash-lite-image; izmjereno 30.07.2026. oko $0.041 po slici jer se izlazna SLIKA naplacuje $30 po milionu tokena, odvojeno od izlaznog teksta; 4:3 daje 1200x896 i oko 1370 tokena; ulazna slika klijenta je 1120 tokena, oko $0.00028, dakle prakticno besplatna), NE zove OLX | Iz poslane fotografije napravi novu sliku artikla: cist prostor, ravno svjetlo, artikal ostaje isti. Recept `pozadina-klijenta` sa postavljenom SLIKOM pozadine ide tokom SLAGANJA (od 04.08.2026.): izrez artikla (@imgly/background-removal-node) + kompozicija na pravu pozadinu (sharp) u kodu, pa Gemini dorada uklapanja; vraca DVIJE slike (`slozena` sa piksel identicnom pozadinom + doradjenu), uvijek 4:3, jedna fotografija artikla, dorada pada mekano (vrati se samo slozena). Od 16.08.2026. se sa TE jedne fotografije prepoznaje i vise stvarnih artikala (`nadjiKomponente`, connected components nad alpha kanalom, jer imgly ne radi instance segmentaciju) i svi se slazu u jedan red po dnu (`rasporediRed`), najvise 4; artikli koji se na fotografiji dodiruju padaju u istu komponentu i broje se kao jedan. Pro modeli su blokirani u kodu (`modelDozvoljen`). Registruje se SAMO kad je `OLX_SLIKA_API_KEY` postavljen. Trosi vanjski AI racun, ne kredite, pa trazi `confirm` i ima dnevni plafon `OLX_SLIKA_MAX_DNEVNO` (slaganje samo ne trosi plafon, dorada trosi 1). Slika pada u `.olx-pik/slike/`, putanja je spremna za `olx_upload_images`. | `recept` | `slike[]` (do 3; za pozadinski recept tacno 1), `logo`, `odnos` (default 4:3; za pozadinski recept samo 4:3), `confirm` | write (fajl na disk, vanjski trosak) | ne | ne |
| `olx_pozadina` | ne zove nista (`core/pozadina.ts`, stanje u `.olx-pik/pozadina/`) | Stalna pozadina klijenta koju koristi recept `pozadina-klijenta`: opis rijecima, slika, ili oboje, plus polozaj artikla (slot: dno sredina, `sirina_artikla_posto`, `margina_dna_posto`). Slika se KOPIRA u klon, pa original iz Telegram inboxa smije nestati. Ide u OBA profila; opis prolazi isti filter kao `dopuna`. Sa SLIKOM pozadine artikal se SLAZE u kodu (slozena verzija piksel identicna, doradjena bez garancije za logo); pozadina zadana samo opisom se crta iznova (slicna, nikad ista, logo iskrivljen). Pri postavljanju upozorava kad slika nije blizu 4:3 (sijece se centar) ili je preuska. Registruje se samo kad je `OLX_SLIKA_API_KEY` postavljen. | `radnja` (`postavi`/`prikazi`/`ukloni`) | `opis` (do 200 znakova), `slika`, `sirina_artikla_posto`, `margina_dna_posto` | write (fajl na disk) | ne | ne |
| `olx_stock_slika` | Wikimedia Commons `api.php` (pretraga) + `upload.wikimedia.org` (preuzimanje), `core/stock-slika.ts`; NE zove ni OLX ni AI | Referentna (stock) fotografija za NOV, ZAPAKOVAN artikal poznatog modela, kad klijent nema sta da slika osim kutije. Vlasnicka odluka od 16.08.2026, zato iza prekidaca `OLX_STOCK_SLIKE` koji je po defaultu UGASEN (bez njega alata nema ni u kontekstu). Izvor je Commons jer svaka slika tamo nosi eksplicitnu slobodnu licencu; licenca i autor se vracaju uz svakog kandidata i moraju se pokazati korisniku, jer je atribucija USLOV koristenja. Brane u jezgru, sve testirane: allowlista hosta (tacno poredjenje imena, dopuna kroz `OLX_STOCK_HOSTOVI` samo rukom u `.env`), samo `https`, bez korisnika i lozinke u URL-u, preusmjerenja se NE prate, sadrzaj se provjerava po magic bajtovima a ne po ekstenziji ni `content-type` zaglavlju (SVG se odbija), granica `MAX_BAJTA_SLIKE` 10 MB. Brana stanja: `used` se odbija, `new` sa oglasa prolazi, nepoznato stanje trazi izricito `stanje: new` PLUS `confirm` (isti obrazac kao nepoznata cijena). Trag (pojam, izvorni URL, licenca, autor, stanje) ide u `.olx-pik/slike-zahtjevi.jsonl`, i kad je zahtjev odbijen; u `audit.jsonl` NE ide jer taj dnevnik prati OLX HTTP pozive i potrosnju kredita. Ide u OBA profila: odluku o tudjoj slici donosi klijent kao oglasivac, i on nosi odgovornost za pravo koristenja. Slike padaju u `.olx-pik/slike/`, spremne za `olx_upload_images` ili kao ulaz u `olx_generiraj_sliku`; AI se ne zove i plafon generisanja se ne trosi. Pokrivenost je ogranicena na poznate modele. | `pojam` | `stanje` (`new`/`used`), `oglas_id` (stanje sa oglasa pobjedjuje zadano), `confirm` | write (fajl na disk) | ne | ne |
...
| `olx_finish_listing` (`server.ts:667`) | POST `/listings/:id/finish` (`core/index.ts:341`) | Oznacava oglas kao zavrsen ili prodan, cuva historiju. | `id` | nema | write | ne | ne postoji endpoint koji zavrsen oglas vraca u aktivne: za `hide` postoji `unhide`, za `finish` nema para. Povratak je nova objava kroz `olx_reaktiviraj_oglas`, sto gubi preglede i pitanja. Da li POST `/listings/:id/publish` nad zavrsenim oglasom ista radi NIJE izmjereno; grana za to stoji u kodu (`planReaktivacije`, opcija `publishRadiNaFinished`) i ugasena je dok mjerenje ne prodje |
...
| `olx_reaktiviraj_oglas` (dodan 16.08.2026.) | GET `/listings/:id`, pa create+upload+publish iz arhive (`core/index.ts` `objaviIzArhive`) | Vraca ZAVRSEN oglas u prodaju objavom NOVOG oglasa sa istim podacima i originalnim slikama sa CDN-a. Odluku donosi cista funkcija `planReaktivacije` (`core/arhiva.ts`): aktivan, istekao, nacrt i nepoznat status se ODBIJAJU sa uputom na jeftiniji put (obnova, izdvajanje, `olx_publish_listing`), skriven a nezavrsen ide na `unhide`, a samo izricit status `finished` ide u ponovnu objavu. Zavrsen oglas zna vratiti cijenu 0 ili "na upit", pa se bez upotrebljive cijene stane i trazi parametar `cijena`. Kad se sa zavrsenog oglasa ne moze preuzeti nijedna slika, koristi se spisak iz ranije arhive (`arhivirajIzZivog` spaja: tekst svjez sa oglasa, slike iz arhive). | `id` | `cijena`, `confirm` (naplatne kategorije), `ignorisi_prethodnu_objavu`, `potvrdi_spornu_robu` | write | da, kad je objava u naplatnoj kategoriji (ista brana kao create/publish) | nova objava se moze zavrsiti ili sakriti; PREGLEDI I PITANJA STAROG OGLASA SE NE PRENOSE |
